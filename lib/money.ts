export type Account={id:string;name:string;kind:'spending'|'savings';balance:number};
export type Routine={id:string;name:string;kind:'income'|'expense'|'transfer';amount:number;frequency:'once'|'weekly'|'biweekly'|'monthly'|'semimonthly';date:string;accountId:string;toId?:string};
...
export type Loan={id:string;name:string;balance:number;apr:number;payment:number;dueDay:number;accountId:string};
...
export function forecast(s:State,days:number){
 ...
 for(let i=1;i<=days;i++){
  ...
  for(const r of s.routines)if(occurs(r,date)&&r.accountId in balances){
...
export function validState(s:any):s is State{
 ...
 for(const a of s.accounts){if(!text(a.id)||ids.has(a.id)||!text(a.name)||!a.name.trim()||!['spending','savings'].includes(a.kind)||!money(a.balance))return false;ids.add(a.id)}
 for(const r of s.routines)if(!text(r.id)||!text(r.name)||!r.name.trim()||!['income','expense','transfer'].includes(r.kind)||!money(r.amount)||r.amount<=0||!['once','weekly','biweekly','monthly','semimonthly'].includes(r.frequency)||!validDate(r.date)||!ids.has(r.accountId)||(r.kind==='transfer'&&(!ids.has(r.toId)||r.toId===r.accountId)))return false;
 ...
 if(s.loans!==undefined){
  ...
  for(const l of s.loans){
   if(!text(l.id)||loanIds.has(l.id)||!text(l.name)||!l.name.trim()||!money(l.balance)||l.balance<0||!money(l.apr)||l.apr<0||l.apr>100||!money(l.payment)||l.payment<=0||!Number.isInteger(l.dueDay)||l.dueDay<1||l.dueDay>31||!s.accounts.some((a:any)=>a.id===l.accountId&&a.kind==='spending'))return false;
   loanIds.add(l.id);
  }
 }
